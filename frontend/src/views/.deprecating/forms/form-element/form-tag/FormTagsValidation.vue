<template>
  <b-card-code title="Tag validation">
    <b-card-text>
      <span>You can optionally provide a tag validator method via the </span> <code>tag-validator</code> <span> prop.</span>
    </b-card-text>
    
    <div>
      <b-form-group :state="state" label="Tags validation example" label-for="tags-validation">
        <b-form-tags v-model="tags" :input-attrs="{ 'aria-describedby': 'tags-validation-help' }" :state="state" :tag-validator="tagValidator" input-id="tags-validation" separator=" " />
