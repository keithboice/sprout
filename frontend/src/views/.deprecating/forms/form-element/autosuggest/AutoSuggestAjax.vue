<template>
  <b-card-code title="Ajax">
    <vue-autosuggest ref="autocomplete" v-model="query" :get-suggestion-value="getSuggestionValue" :input-props="inputProps" :render-suggestion="renderSuggestion" :section-configs="sectionConfigs" :suggestions="suggestions" @input="fetchResults" />
  ...
  methods: {
    fetchResults () {
      ...
      this.timeout = setTimeout( () => {
        ...
        Promise.all( [ photosPromise, usersPromise ] )
          .then( values => {
            this.suggestions = []
            this.selected = null
            ...
            users.length && this.suggestions.push( {
                                                     name: "destinations",
                                                     data: users
                                                   } )
            photos.length && this.suggestions.push( {
                                                      name: "hotels",
                                                      data: photos
                                                    } )
          } )
