<template>
  <b-card-code title="Section">
    <vue-autosuggest v-model="query" :input-props="inputProps" :limit="10" :render-suggestion="renderSuggestion" :section-configs="sectionConfigs" :suggestions="suggestions" @input="fetchResults" />
  ...
  methods: {
    fetchResults ( text ) {
      ...
      const filteredDevelopers = this.datasuggest.data[ 0 ].developers.filter( item => item.name.toLowerCase()
        ...
        .slice( 0, this.limit )
      const filteredPages = this.datasuggest.data[ 0 ].pages.filter( item => item.name.toLowerCase()
                                                                               .indexOf( text.toLowerCase() ) > -1 )
        .slice( 0, this.limit )
