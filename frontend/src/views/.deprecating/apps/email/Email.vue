<template>
  ...
  <div style="height: inherit">
    ...
    <div class="email-app-list">
      ...
      <div class="app-action">
        <div class="action-left">
          <b-form-checkbox :checked="selectAllEmailCheckbox" :indeterminate="isSelectAllEmailCheckboxIndeterminate" @change="selectAllCheckboxUpdate">
            Select All
          </b-form-checkbox>
        </div>
        <div v-show="selectedEmails.length" :class="{'d-flex': selectedEmails.length}" class="align-items-center">
          
          <!-- Update Mail Folder Dropdown -->
          <b-dropdown no-caret right toggle-class="p-0" variant="link">
            <template #button-content>
              <feather-icon class="align-middle text-body" icon="FolderIcon" size="17" />
            </template>
          ...
          <!-- Update Mail Folder Dropdown -->
          <b-dropdown class="ml-1" no-caret right toggle-class="p-0" variant="link">
            <template #button-content>
          ...
          <feather-icon class="cursor-pointer ml-1" icon="MailIcon" size="17" @click="markSelectedEmailsAsUnread" />
          
          <feather-icon v-show="$route.params.folder !== 'trash'" class="cursor-pointer ml-1" icon="TrashIcon" size="17" @click="moveSelectedEmailsToFolder('trash')" />
            ...
            <b-media-aside class="media-left mr-50">
              ...
              <div class="user-action">
                ...
                <div class="email-favorite">
                  <feather-icon :class="{ 'text-warning fill-current': email.isStarred }" icon="StarIcon" size="17" @click.stop="toggleStarred(email)" />
                </div>
              </div>
            </b-media-aside>
            ...
            <b-media-body>
              <div class="mail-details">
                ...
                <div class="mail-meta-item">
                  ...
                  <span v-for="label in email.labels" :key="label" :class="`bullet-${resolveLabelColor(label)}`" class="mx-50 bullet bullet-sm" />
                  <span class="mail-date">{{ formatDateToMonthShort( email.time, {
                    hour:   "numeric",
                    minute: "numeric"
                  } ) }}</span>
                </div>
    ...
    <!-- Email View/Detail -->
    <email-view :class="{'show': showEmailDetails}" :email-view-data="emailViewData" :opended-email-meta="opendedEmailMeta" @close-email-view="showEmailDetails = false" @move-email-to-folder="moveOpenEmailToFolder" @toggle-email-starred="toggleStarred(emailViewData)" @update-email-label="updateOpenEmailLabel" @mark-email-unread="markOpenEmailAsUnread" @change-opened-email="changeOpenedEmail" />
    
    <!-- Sidebar -->
    <portal to="content-renderer-sidebar-left">
      <email-left-sidebar :class="{'show': mqShallShowLeftSidebar}" :emails-meta="emailsMeta" :shall-show-email-compose-modal.sync="shallShowEmailComposeModal" @close-left-sidebar="mqShallShowLeftSidebar = false" />
    </portal>
...
import {
  BFormInput, BInputGroup, BInputGroupPrepend, BDropdown, BDropdownItem, BFormCheckbox, BMedia, BMediaAside, BMediaBody, BAvatar
}                                                from "bootstrap-vue"
import VuePerfectScrollbar                       from "vue-perfect-scrollbar"
import {
  filterTags, formatDateToMonthShort
}                                                from "@utils/filter"
...
export default {
  ...
  setup () {
    ...
    const selectAllEmailCheckbox = computed( () => emails.value.length && (
...
<style lang="scss">
@import "~@scss/base/pages/app-email.scss";
</style>
