    ...
    <div class="email-detail-header">
      ...
      <div class="email-header-left d-flex align-items-center">
        <span class="go-back mr-1">
          <feather-icon :icon="$store.state.chat.isRTL ? 'ChevronRightIcon' : 'ChevronLeftIcon'" class="align-bottom" size="20" @click="$emit('close-email-view')" />
        </span>
      ...
      <div class="email-header-right ml-2 pl-1">
        ...
        <!-- Mark Starred -->
        <feather-icon :class="{ 'text-warning fill-current': emailViewData.isStarred }" class="cursor-pointer" icon="StarIcon" size="17" @click="$emit('toggle-email-starred')" />
        
        <!-- Move email to folder -->
        <b-dropdown class="ml-75" no-caret right toggle-class="p-0" variant="link">
        ...
        <!-- Mark Un-Read -->
        <feather-icon class="ml-75 cursor-pointer" icon="MailIcon" size="17" @click="$emit('mark-email-unread')" />
        
        <!-- Move to Trash -->
        <feather-icon v-show="$route.params.folder !== 'trash'" class="ml-75" icon="TrashIcon" size="17" @click="$emit('move-email-to-folder', 'trash')" />
        
        <!-- Show Previous Mail -->
        <feather-icon :class="{'text-muted pointer-events-none': !opendedEmailMeta.hasPreviousEmail}" :icon="$store.state.chat.isRTL ? 'ChevronRightIcon' : 'ChevronLeftIcon'" class="ml-75 cursor-pointer" size="17" @click="$emit('change-opened-email', 'previous')" />
        
        <!-- Show Next Mail -->
        <feather-icon :class="{'text-muted pointer-events-none': !opendedEmailMeta.hasNextEmail}" :icon="$store.state.chat.isRTL ? 'ChevronLeftIcon' : 'ChevronRightIcon'" class="ml-75 cursor-pointer" size="17" @click="$emit('change-opened-email', 'next')" />
      </div>
    </div>
      ...
      <b-row>
        <b-col cols="12">
          <div class="email-label">
            <b-badge v-for="(label) in emailViewData.labels" :key="label" :variant="`light-${resolveLabelColor(label)}`" class="text-capitalize mr-50" pill>
              {{ label }}
            </b-badge>
          </div>
        </b-col>
