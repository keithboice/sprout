<template>
  <div>
    ...
    <b-media class="mb-2">
      <template #aside>
        <b-avatar ref="previewEl" :src="userData.avatar" :text="avatarText(userData.fullName)" :variant="`light-${resolveUserRoleVariant(userData.role)}`" rounded size="90px" />
      </template>
      <h4 class="mb-1">
        {{ userData.fullName }} </h4>
      <div class="d-flex flex-wrap">
        <b-button variant="primary" @click="$refs.refInputEl.click()">
          <input ref="refInputEl" class="d-none" type="file" @input="inputImageRenderer"> <span class="d-none d-sm-inline">Update</span>
      ...
      <b-row>
        ...
        <b-col cols="12" md="4">
          <b-form-group label="Status" label-for="user-status">
            <v-select v-model="userData.status" :clearable="false" :dir="$store.state.chat.isRTL ? 'rtl' : 'ltr'" :options="statusOptions" :reduce="val => val.value" input-id="user-status" />
          </b-form-group>
        </b-col>
    ...
    <!-- PERMISSION TABLE -->
    <b-card class="border mt-1" no-body>
      ...
      <b-table :items="permissionsData" class="mb-0" responsive striped>
    ...
    <!-- Action Buttons -->
    <b-button :block="$store.getters['app/currentBreakPoint'] === 'xs'" class="mb-1 mb-sm-0 mr-0 mr-sm-1" variant="primary">
      Save Changes
    </b-button>
...
import useUsersList              from "@/views/.deprecating/user/users-list/useUsersList"
