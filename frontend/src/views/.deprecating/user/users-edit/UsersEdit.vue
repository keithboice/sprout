<template>
  <component :is="userData === undefined ? 'div' : 'b-card'">
    ...
    <!-- Alert: No item found -->
    <b-alert :show="userData === undefined" variant="danger">
      <h4 class="alert-heading">
        Error fetching user data </h4>
      <div class="alert-body">
        No user found with this user id. Check
        <b-link :to="{ name: 'apps-users-list'}" class="alert-link">
          User List
        </b-link>
        for other users.
      </div>
    ...
    <b-tabs v-if="userData" pills>
      ...
      <b-tab>
        <template #title>
          <feather-icon class="mr-0 mr-sm-50" icon="Share2Icon" size="16" />
