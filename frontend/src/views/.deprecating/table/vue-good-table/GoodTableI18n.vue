  ...
  <b-card-code title="I18n Table">
    ...
    <div class="custom-search d-flex justify-content-end">
      <b-form-group>
        <div class="d-flex align-items-center">
          ...
          <b-form-input v-model="searchTerm" :placeholder=" $t('message.SearchPlaceholder')" class="d-inline-block" type="text" />
        </div>
      </b-form-group>
    </div>
    ...
    <vue-good-table :columns="columns" :pagination-options="{
      ...
      <template slot="table-column" slot-scope="props">
        ...
        </span> <span v-else-if="props.column.label ==='Email'" class="text-nowrap">
          {{ $t( "message.tableHeader.email" ) }}
        </span> <span v-else-if="props.column.label ==='Date'" class="text-nowrap">
          {{ $t( "message.tableHeader.date" ) }}
        </span> <span v-else-if="props.column.label ==='Salary'" class="text-nowrap">
          {{ $t( "message.tableHeader.salary" ) }}
        </span> <span v-else-if="props.column.label ==='Status'" class="text-nowrap">
          {{ $t( "message.tableHeader.status" ) }}
        </span> <span v-else-if="props.column.label ==='Action'" class="text-nowrap">
      ...
      <template slot="pagination-bottom" slot-scope="props">
        <div class="d-flex justify-content-between flex-wrap">
          ...
          <div>
            <b-pagination :per-page="pageLength" :total-rows="props.total" :value="1" align="right" class="mt-1 mb-0" first-number last-number next-class="next-item" prev-class="prev-item" @input="(value)=>props.pageChanged({currentPage:value})">
              <template #prev-text>
...
import { BAvatar, BBadge, BPagination, BFormGroup, BFormInput, BFormSelect, BAlert, BDropdown, BDropdownItem } from "bootstrap-vue"
