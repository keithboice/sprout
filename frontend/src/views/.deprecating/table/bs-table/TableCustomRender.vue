<template>
  <b-card-code no-body title="Custom data rendering">
    <b-table :fields="fields" :items="items" class="mb-0" responsive>
      ...
      <template #cell(Popularity)="data">
        <b-progress :value="data.value.value" :variant="data.value.variant" height="7px" max="100" striped />
      </template>
      
      <template #cell(order_status)="data">
        <b-badge :variant="data.value.variant" pill>
          {{ data.value.status }}
        </b-badge>
      </template>
