  ...
  emails: [
    {
      ...
      subject:     "Profound contacts-facing frame",
      cc:          [],
      bcc:         [],
...
mock.onGet( "/apps/email/emails" )
  .reply( config => {
    ...
    const filteredData = data.emails.filter( email => /* eslint-disable operator-linebreak, implicit-arrow-linebreak */
                                               email.subject.toLowerCase()
                                                 .includes( queryLowered ) && isInFolder( email ) && (
...
// POST: Update Email
// ------------------------------------------------
mock.onPost( "/apps/email/update-emails" )
  .reply( config => {
    const {
            emailIds,
            dataToUpdate
...
// ------------------------------------------------
mock.onPost( "/apps/email/update-emails-label" )
  .reply( config => {
    const {
            emailIds,
            label
...
// ------------------------------------------------
mock.onGet( "/apps/email/paginate-email" )
  .reply( config => {
    const {
            dir,
            emailId
