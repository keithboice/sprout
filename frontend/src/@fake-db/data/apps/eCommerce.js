...
import { getRandomInt, paginateArray, randomDate, sortCompare } from "@/@fake-db/utils";
  ...
  products:     [
    {
      ...
      description:     "FIND KEYS, BAGS & MORE -- Pro is our high-performance finder ideal for keys, backpacks, luggage or any other items you want to keep track of. The easy-to-use finder and free styles work with iOS and Android.",
      brand:           "Tile",
      price:           29.99,
...
// GET: Return products
// ------------------------------------------------
mock.onGet( "/apps/dashboard-ecomm/products" )
  .reply( config => {
    // eslint-disable-next-line object-curly-newline
    const {
            q       = "",
    ...
    const filteredData = data.products.filter( product => product.name.toLowerCase()
...
// ------------------------------------------------
mock.onGet( /\/apps\/ecommerce\/products\/\d+/ )
  .reply( config => {
...
// ------------------------------------------------
mock.onGet( "/apps/dashboard-ecomm/wishlist" )
  .reply( () => {
...
// ------------------------------------------------
mock.onGet( "/apps/dashboard-ecomm/cart" )
  .reply( () => {
...
// ------------------------------------------------
// POST: Add Item in contacts Cart
// ------------------------------------------------
mock.onPost( "/apps/dashboard-ecomm/cart" )
  .reply( config => {
...
// ------------------------------------------------
// DELETE: Remove Item from contacts Cart
// ------------------------------------------------
mock.onDelete( /\/apps\/ecommerce\/cart\/\d+/ )
  .reply( config => {
    // Get product id from URL
    let productId = config.url.substring( config.url.lastIndexOf( "/" ) + 1 );
...
// ------------------------------------------------
// POST: Add Item in contacts Wishlist
// ------------------------------------------------
mock.onPost( "/apps/dashboard-ecomm/wishlist" )
  .reply( config => {
    // Get product from post data
    const { productId } = JSON.parse( config.data );
...
// ------------------------------------------------
// DELETE: Remove Item from contacts Wishlist
// ------------------------------------------------
mock.onDelete( /\/apps\/ecommerce\/wishlist\/\d+/ )
  .reply( config => {
    // Get product id from URL
    let productId = config.url.substring( config.url.lastIndexOf( "/" ) + 1 );
